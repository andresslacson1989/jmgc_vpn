...
install_squid() {
    #send Squid
    curl -sb -X POST https://jmgc-projects.store/api/server/install -H "Content-Type: application/x-www-form-urlencoded" -d "status=Squid&ip=$IP"
    clear
...
#!/bin/bash
##Authentication
data=$(curl -sb -X POST https://jmgc-projects.store/api/server/login -H "Content-Type: application/x-www-form-urlencoded" -d "login=true&username=$username&password=$password")
...
data=$(curl -sb -X POST https://jmgc-projects.store/api/server/update-status -H "Content-Type: application/x-www-form-urlencoded" -d "status=1&username=$common_name")
...
data=$(curl -sb -X POST https://jmgc-projects.store/api/server/update-status -H "Content-Type: application/x-www-form-urlencoded" -d "status=0&username=$common_name")
BYTES2
